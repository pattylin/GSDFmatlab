...
r = 0.03 %data: 0.03-0.05 syn: 0.01;
isfigure = 1;
...
for ie = 1:length(phvmatfiles)
	...
	disp(eventphv(1).id);
	evla(ie) = eventphv(1).evla;
	evlo(ie) = eventphv(1).evlo;
	%evdp(ie) = eventphv(1).evdp;
    eventinfo(ie).evla = eventphv(1).evla;
    eventinfo(ie).evlo= eventphv(1).evlo;
    %eventinfo(ie).evdp= eventphv(1).evdp;

    eventinfo(ie).id= eventphv(1).id;
...
for ip=1:length(periods)
	for i = 1:Nx
		for j=1:Ny
			...
			ind = find( abs(GV_mat(i,j,:,ip) - avgphv(ip).GV(i,j)) > 0.05*avgphv(ip).GV(i,j));
...
save GV_mat4plot.mat  GV_mat
save useEVT.mat eventinfo

%save(['eikonal_stack_',comp,'.mat'],'avgphv','GV_mat','ori_GV_mat','evla','evlo','evdp');
