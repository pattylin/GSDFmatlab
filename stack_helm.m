...
fid = fopen('ueventlist.txt','w'); % NJA
for ie = 1:length(phvmatfiles)
	temp = load([phase_v_path,phvmatfiles(ie).name]);
	helmholtz = temp.helmholtz;
    eventinfo(ie).evla = helmholtz(1).evla;
    eventinfo(ie).evlo= helmholtz(1).evlo;
    eventinfo(ie).evdp= helmholtz(1).evdp;
    
    eventinfo(ie).id= helmholtz(1).id;
	...
	for ip=1:length(periods)
		...
		if helmholtz(ip).goodnum./helmholtz(ip).badnum < min_csgoodratio
            ...
            % Print out the used events and the periods at which they are
            % used
            fprintf(fid,'%4.1f %s\n',[periods(ip) helmholtz(1).id]);
...
save GV_cor_mat4plot.mat  GV_cor_mat
